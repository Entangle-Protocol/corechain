...
error EntangleToken__E10();    // Chain is not supported on a bridge

/// @title EntangleToken
/// @notice Represents an *NGL token used for bridging operations between different chains.
contract EntangleToken_V2 is Initializable, ERC20Upgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function setEndPoint(address _as) external onlyRole(ADMIN) {
    ...
    event BridgeDone(address to, uint256 amount, bytes32[2] txhash, uint256 fromChain, bytes32 indexed marker);
    ...
    function bridge(uint256 _chainIdTo, bytes memory _to, uint256 _amount, bool unwrap, bytes32 _marker) external whenNotPaused {
        ...
        if (minBridgeAmounts[_chainIdTo] == 0) revert EntangleToken__E10();
        ...
        aggregationSpotter.propose(
            ...
            eobChainId,
            bridgeRouterAddress,
            PhotonFunctionSelectorLib.encodeEvmSelector(bytes4(keccak256("bridge(bytes)"))),
